...
struct Cli {
    ...
    /// Disable local pattern elimination
    #[arg(long, verbatim_doc_comment)]
    not_use_lpe: bool,

    /// Disable biclique heuristic merging
    #[arg(long, verbatim_doc_comment)]
    not_use_bhm: bool,

    /// Disable zero-node pruning
    #[arg(long, verbatim_doc_comment)]
    not_use_znp: bool,
...
fn main() {
    ...
    let mut ddnnf: Ddnnf;
    let mut not_use_lpe: bool = false;
    let mut not_use_bhm: bool = false;
    let mut not_use_znp: bool = false;
    ...
    if cli.pipe_ddnnf_stdin {
        ...
        ddnnf = dparser::distribute_building(input, cli.total_features, None,not_use_lpe,not_use_bhm,not_use_znp);
    } else {
        ...
        ddnnf = dparser::my_build_ddnnf(ddnnf_path, cli.total_features,m,n,iter,not_use_lpe,not_use_bhm,not_use_znp);
    ...
    if cli.operation.is_some() {
        ...
        match &operation {
            ...
            Count { features } => {
                ...
                if cli.not_use_znp {
                    println!(
                        ...
                        &features,
                        ddnnf.raw_execute_query(&features)
                    );
                }else{
                    println!(
                        "\nDdnnf count for query {:?} is: {:?}",
            ...
            CountQueries {
                ...
                if cli.not_use_znp {
                    ...
                    compute_queries(
                        ...
                        queries_input_file,
                        &output_file_path,
                        Ddnnf::my_execute_partial_config,
                    );
                }
            ...
            Weighted {
                ...
                ddnnf.limit_solution_num = 1;
                let mut list:Vec<i32> = Vec::new();
                let start = Instant::now();
                let res = ddnnf.my_weighted_query(&list);
            ...
            Project {
                ...
                for len in 2..102 {
                    ...
                    let mut temp_ddnnf = ddnnf.clone();
                    let unexc_limit = 200;

                    let start = Instant::now();
                    let res1 = temp_ddnnf.new_my_project_query_unexc(&list, &vec![],unexc_limit);
                    let res2 = temp_ddnnf.my_project_query_unexc(&list, &vec![],unexc_limit);
                    // temp_ddnnf.limit_solution_num = 10000;
                    // let res3 = temp_ddnnf.my_project_query(&list, &vec![]);
                    
                    // let mix_res = min(res1.0, res2.0);
                    println!("{:?}, {:?}",res1,res2);
                ...
                // let unexc_limit = 20000;
                // temp_ddnnf = ddnnf.clone();
                // let start = Instant::now();
                // let res1 = temp_ddnnf.new_my_project_query_unexc(&list, &vec![],unexc_limit);
                // let res2 = temp_ddnnf.my_project_query_unexc(&list, &vec![],unexc_limit);
                // let res3 = temp_ddnnf.my_project_query_unexc_dev(&list,&vec![],unexc_limit*1000);
                // ddnnf.limit_solution_num = 10000;
                // let res4 = ddnnf.my_project_query(&list, &vec![]);

                // println!("{:?}",res1);
                // println!("{:?}",res2);
                // println!("{:?}",res3);
                // println!("{:?} {:?}",res4.0.len(),res4.1);
