...
#[inline]
pub fn build_ddnnf(mut path: &str, mut total_features: Option<u32>,not_use_lpe: bool,not_use_bhm: bool,not_use_znp:bool ) -> Ddnnf {
    ...
    if clauses.is_empty() {
        distribute_building(lines, total_features, None,not_use_lpe,not_use_bhm,not_use_znp)
    } else {
        distribute_building(lines, total_features, Some(clauses),not_use_lpe,not_use_bhm,not_use_znp)
    }
}
...
pub static mut GLOBAL_T: u32 = 3;


#[inline]
pub fn my_build_ddnnf(mut path: &str, mut total_features: Option<u32>, m: u32,n: u32,t:u32,not_use_lpe: bool, not_use_bhm:bool,not_use_znp:bool) -> Ddnnf {
    unsafe {
        GLOBAL_M = m;
        GLOBAL_N = n;
        GLOBAL_T = t;
        ...
        Ok(_) | Err(_) => {
            ...
            match total_features {
                Some(o) => {
                    ...
                    if not_use_lpe == false {
                        build_d4_ddnnf(lines, total_features, clauses,not_use_lpe,not_use_bhm,not_use_znp)
                    }else{
                        raw_build_d4_ddnnf(lines, total_features, clauses,not_use_lpe,not_use_bhm,not_use_znp)
                    }
                }
                None => {
...
fn raw_build_d4_ddnnf(
    ...
    println!("raw smooth time = {:?}", start.elapsed());
...
fn build_d4_ddnnf(
    ...
    let mut is_and:Vec::<bool> = Vec::new();
    is_and.resize(lines.len(),false);
    ...
    println!("smooth time = {:?}", duration);

    let start = Instant::now();

    let mut tuopu_list: Vec<NodeIndex> = Vec::new();
    let mut erase_flag: Vec<bool> = vec![false; ddnnf_graph.node_count()];

    tuopu_list = get_tuopu_list(&mut ddnnf_graph,&mut set);

    // println!("{:?}",tuopu_list);

    for nx in tuopu_list.iter().rev(){
        if nx.index() < set.capacity() && set[nx.index()] {
            continue;
        }
        match ddnnf_graph[*nx] {
            TId::And => {
                let mut parent_count = 0;
                let mut parent:NodeIndex = NodeIndex::new(0);
                for edge in ddnnf_graph.edges_directed(*nx, Incoming) {
                    if edge.source().index() < set.capacity() && set[edge.source().index()] {
                        continue;
                    }
                    parent_count += 1;
                    parent = edge.source();
                ...
                if parent_count == 1 {
                    match ddnnf_graph[parent] {
                        TId::And => {
                            erase_flag[nx.index()] = true;
                        }
                        _ => {}
                    }
                }
    ...
    let mut fa: Vec<NodeIndex> = vec![NodeIndex::new(0); ddnnf_graph.node_count()];
    for nx in tuopu_list.iter() {
        fa[nx.index()] = *nx;
    }
    for nx in tuopu_list.iter() {
        for edge in ddnnf_graph.edges_directed(*nx, Outgoing) {
            let to = edge.target();
            if to.index() < set.capacity() && set[to.index()] {
                continue;
            }
            if erase_flag[to.index()] == true {
                fa[to.index()] = fa[nx.index()];
            }
        }
    }
    let mut edges_to_add: Vec<(NodeIndex, NodeIndex)> = Vec::new();
    for nx in ddnnf_graph.node_indices() {
        if ( nx.index() < set.capacity() && set[nx.index()] )|| erase_flag[nx.index()] {
            continue;
        }
        for edge in ddnnf_graph.edges_directed(nx, Incoming) {
            let f = edge.source();
            if f.index() < set.capacity() && set[f.index()] {
                continue;
            }
            if erase_flag[f.index()] {
                edges_to_add.push((fa[f.index()],nx));
            }
        }
    }
    ...
    let mut num = 0;
    for nx in ddnnf_graph.node_indices() {
        if erase_flag[nx.index()] {
            num += 1;
            set[nx.index()] = true;
        }
    }
    // println!("erase_flag_num: {:?}",num);

    for (parent,son) in edges_to_add {
        ddnnf_graph.add_edge(parent, son, ());
    }  

    // println!("erase_flag_time: {:?}", start.elapsed());
    ...
    // let duration = start.elapsed();
    let mut ddnnf = Ddnnf::new(parsed_nodes, literals, true_nodes, total_features, clauses);
    return ddnnf;
}
...
fn get_tuopu_list (
    ...
    let mut du: Vec<i32> = vec![0;ddnnf_graph.node_count() + 1];
    ...
    while !q.is_empty() {
        ...
        tuopu_list.push(top);
        for edge in ddnnf_graph.edges_directed(top, Outgoing) {
            if edge.target().index() < set.capacity() && set[edge.target().index()] {
                continue;
            }
            du[edge.target().index()] -= 1;
            if du[edge.target().index()] == 0 {
...
fn merge_edge(
    ...
    for k in 1..t+1 {
        ...
        // let merge_num = m as usize;
        // let group_num = n as usize;
            ...
            for i in (group_num - 1..vlist.len()-1).rev().step_by(group_num) {
                ...
                for neighbor in ddnnf_graph.neighbors_directed(node_list[0], Outgoing){
                    if flag[neighbor.index()] == group_num {
                        num += 1;
                        let son_list = ddnnf_graph.neighbors_directed(neighbor, Outgoing);
                        let mut son_num = 0;
                        for _ in son_list {
                            son_num += 1;
                        }
    ...
    // println!("non_leaf_num: {:?}",non_leaf_num);
