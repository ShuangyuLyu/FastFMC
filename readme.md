# FastFMC: An Optimized DNNF reuse Method for Efficient Feature Model Counting

## Reference
FastFMC builds upon [d-dnnf-reasoner](https://github.com/SoftVarE-Group/d-dnnf-reasoner). Our implementation takes that codebase as the starting point and extends/modifies it to obtain the FastFMC tool described in this repository. FastFMC is distributed under the same LGPL-3.0 license as d-dnnf-reasoner; see [LICENSE](LICENSE).
...
The directory `experimental_results/` contains 11 folders for presenting the results of all experiments. 
...
+ [hyper_parameter](experimental_results/hyper_parameter/): Results of FastFMC with different μ settings (μ=1, μ=5, μ=10, μ=30, μ=50, μ=100) on all testing benchmarks.
...
+ [memory](experimental_results/memory/): Results of peak memory usage (peak RSS) for all tools. `memory_per_benchmark.csv` gives the per-benchmark peak memory (GB) for each tool, with `NA` for benchmarks the tool did not solve.
